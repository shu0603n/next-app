...
export default function AlertCustomerDelete({ employee_id, date, open, handleClose, onReload }: Props) {
  const handleClick = (isDelete: boolean) => {
    if (isDelete) {
      fetch(`/api/db/attendance/delete?employee_id=${employee_id}&date=${date}`)
        .then((response) => {
          if (!response.ok) {
