    ...
    onSubmit: (values, { setSubmitting }) => {
      try {
        if (customer) {
          ...
          fetch(`/api/db/parameter/project_position/update?id=${values.id}&name=${values.name}&description=${values.description}`)
            .then((response) => {
              if (!response.ok) {
          ...
          alertSnackBar('処理中…', 'secondary');
          fetch(`/api/db/parameter/project_position/insert?name=${values.name}&description=${values.description}`)
            .then((response) => {
              if (!response.ok) {
                throw new Error('更新に失敗しました。');
              }
                    ...
                    <Grid item xs={12}>
                      <Stack spacing={1.25}>
                        ...
                        <TextField
                          fullWidth
                          id="description"
                          placeholder={customer !== null ? customer.description : 'パラメーターを入力してください'}
